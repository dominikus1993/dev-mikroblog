...
internal class GetPostsUseCase
{
    ...
    {
        return await _postProvider.GetPosts(new GetPostQuery(query.Page, query.PageSize, query.Tag, query.AuthorId), cancellationToken).Select(x => PostDto.FromPost(x)).ToListAsync(cancellationToken);
    }
}